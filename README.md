...
* [`my-voice-tree.py`](my-voice-tree.py): A voice-controlled tree that you can change the lights on in a number of ways [by speaking to it](https://www.youtube.com/watch?v=oPNQIW4BptQ). Supported commands include "christmas tree blue|red|green|white" to change the tree to the corresponding colours, "christmas tree phase" to cycle through LED hues, "christmas tree disco" to revert to the same pattern used by `my-tree.py`.  It is also possible to get the tree to speak using "christmas tree speak".  It's also possible to get the tree to 'sing' and play an mp3 of I Wish It Could Be Christmas Every Day while lit up.  To generate a new utterance "this is my message" use "christmas tree generate this is my message".  The voice tree uses AWS Transcribe and Polly to work its Christmas magic.  The sing feature was inspired by Black Mirror.  The video [here](https://youtu.be/YopBvuAIyBU) shows the tree in action.
